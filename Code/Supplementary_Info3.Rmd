...
# Load data sets
```{r include = FALSE}
load(here("Data", "complete_ts.RData"))
load(here("Data", "pop_aggr.RData"))
interactions_df <- read_csv(here("Data", "interactions_df.csv"))
...
```{r interactions_density,  fig.align="center", fig.height=12, fig.width=10, warning=FALSE}
# plot distribution of mean_interaction faceted by richness, not by nutrients, and use distribution plot with median value

# create column == richness based on the number of letter in the column "composition"
interactions_df<-interactions_df%>%mutate(richness=as.numeric(nchar(composition)))

plot_int_temperature<-ggplot(data=interactions_df,aes(x=coefficient_value))+
  ...
  theme_classic() +
   ...
   xlab("interaction coefficient values") +
  ylab("density")+
...
plot_int_nutrients<-ggplot(data = interactions_df, aes(x = coefficient_value)) +
  geom_density(aes(fill = factor(nutrients)), alpha = 0.5) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  theme_classic() +
  theme(axis.title.x = element_text(size = 20),
        axis.title.y = element_text(size = 20),
...
plot_int_richness<-ggplot(data=interactions_df,aes(x=coefficient_value))+
  geom_density(aes(fill=factor(richness)),alpha=0.5)+
  geom_vline(xintercept = 0,linetype="dashed")+
  theme_classic() +
   theme(axis.title.x = element_text(size = 20),
        axis.title.y = element_text(size = 20),
