...
# Save the Word file
# Save
print(doc, targethere("tables", "anova_comparison1.docx"))
...
```{r effect_RD_nutrients, results='hide', echo=FALSE, warning=FALSE, fig.align="center", fig.height=10, fig.width=20}
plot_main <- main_r_dd %>%
  ggplot(aes(x = (log10(balance)), y = log10(1/CV), color = type)) +
  geom_point(aes(x = log10(balance), y = log10(1/CV)), size = 5, alpha = 0.65) +
  geom_smooth(method = "lm", aes(x = log10(balance), y = log10(1/CV)), linewidth = 3, show.legend=F) +
...
complete_aggr_2_scaled <- complete_aggr_2 %>%
  group_by(nutrients) %>%
  mutate(scaled_stability = scale(log10(stability))) %>%
  ungroup()


ggplot(complete_aggr_2_scaled, aes(
  x = log10(balance_f),
  y = log10(mean_biomass),
  color = scaled_stability,
  shape = as.factor(nutrients))
) +
  geom_point(alpha = 0.8, size = 3) +
  geom_smooth(method = "lm", se = FALSE) +
  scale_color_viridis_c(option = "inferno", begin = 0.1, end = 0.8) +
  theme_bw() +
  labs(
    color = "Scaled Stability\n(within nutrients)",
    shape = "Nutrients",
    x = "log10(Imbalance)",
    y = "log10(Mean Biomass)"
  )
```
