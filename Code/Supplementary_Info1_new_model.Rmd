---
title: "Supplementary Materials 1: The Imbalance of Nature: Critical Role of Species Environmental Responses for Community Stability"
...
complete_aggr<-aggr_ts %>%group_by(composition,nutrients,temperature,sample_ID)%>%reframe(
  ...
  abs_mean_slope=mean(abs_mean_slope),
...
#ggsave(here("figures_ms/fig.2.png"), plot = combined_plot, width = 20, height = 8, dpi = 600)
...
<!-- Low nutrients are strongly destabilising, raising questions about population sizes in low-nutrient conditions. Low populations may experience increased demographic stochasticity, affecting stability. It would be insightful to examine whether stable communities are consistently those with higher biomass, indicating potential destabilizing effects of stochastic events in low-density communities, independent of their imbalance level.    -->
<!-- ```{r} -->


<!-- complete_aggr4 <- complete_ts %>% -->
<!--   group_by(sample_ID, nutrients, composition, richness, predict_spec) %>%  -->
<!--   dplyr::summarize( -->
<!--   species_mean_bm = mean(biomass, na.rm=T), -->
<!--   species_var_bm = var(biomass, na.rm = T), -->
<!--   community_CV = sd(tot_biomass, na.rm=T)/mean(tot_biomass, na.rm = T), -->
<!--   mean_tot_biomass = mean(tot_biomass, na.rm=T) -->
<!--  ) -->

<!-- # Create the plot with slope annotations -->
<!-- ggplot(complete_aggr4, aes(x = species_mean_bm, y = species_var_bm, colour = nutrients)) + -->
...
<!-- ggplot(complete_aggr4, aes(x = mean_tot_biomass, y = community_CV, colour = nutrients)) + -->
<!--   geom_point(alpha = 0.6) + -->
<!--   geom_smooth(method = "lm", se = FALSE) + -->
<!--   scale_x_log10() + -->
<!--   scale_y_log10() + -->
...
complete_aggr <- complete_aggr %>% mutate(stability = 1/CV) %>% 
  dplyr::rename(balance_f = abs_mean_slope,
...
perf_table <- performance(mod1) %>% 
  bind_rows(performance(mod2), .id = "model") %>% 
  flextable()

# Save the table in a Word document for the supplement
doc <- read_docx() %>% 
  body_add_par("Model Performance Comparison", style = "heading 1") %>%
  body_add_flextable(perf_table)

print(doc, target = here("tables", "model_performance.docx"))
...
Now we build a linear model where stability is modeled as a function of imbalance and divergence. 
...
perf_table2 <- performance(lm_div_balance) %>% 
...
doc <- read_docx() %>% 
  body_add_par("Model Performance Comparison", style = "heading 1") %>%
  body_add_flextable(perf_table2)


print(doc, target = here("tables", "model_imb_vs_div.docx"))
...
anova_ft <- anova_tidy1 %>%
  mutate(
    `Sum of Squares` = round(sumsq, 2),
    `DF` = df,
    `F Statistic` = round(statistic, 2),
    `p-value` = format.pval(p.value, digits = 3, eps = .001)
  ) %>%
  rename(Term = term) %>%
  dplyr::select(Term, `DF`, `Sum of Squares`, `F Statistic`, `p-value`) %>%  # <- FIXED HERE
  flextable() %>%
  bold(i = ~ as.numeric(`p-value`) < 0.05, j = "p-value") %>%
  autofit()

# Add to Word document
doc <- read_docx() %>%
  body_add_par("ANOVA Comparison", style = "heading 1") %>%
  body_add_flextable(anova_ft)

# Save the Word file
# Save
print(doc, here("tables", "anova_comparison1.docx"))
...
# Save the Word file
# Save
print(doc, target = here("tables", "anova_comparison2.docx"))
...
# Save the Word file
# Save
print(doc, target = here("tables", "anova_comparison3.docx"))
...
# Save the Word file
# Save
print(doc, target = here("tables", "anova_comparison4.docx"))
...
# Standardize predictors (not response)


df_std <- complete_aggr %>%
  mutate(
    log10_balance_f = scale(log10(balance_f))[,1],
    divergence_std = scale(divergence)[,1]
  )

lm_std <- lm(log10(stability) ~ log10_balance_f + divergence_std, data = df_std)

coef_df <- summary(lm_std)$coefficients[-1, , drop = FALSE] # remove intercept

vi_data <- data.frame(
  Variable = rownames(coef_df),
  Importance = abs(coef_df[, "Estimate"])
)

# Relabel variables
vi_data$Variable <- recode(vi_data$Variable,
                          "log10_balance_f" = "Imbalance (log10)",
                          "divergence_std" = "Divergence")

# Plot

ggplot(vi_data, aes(x = Importance, y = reorder(Variable, Importance))) +
  geom_col(fill = "steelblue") +
  labs(x = "Absolute standardized coefficient", y = NULL) +
  theme_bw()
...
balance_dd<-complete_aggr%>%dplyr::select(balance_f,richness,CV, nutrients)%>%
  cbind(type="fundamental")%>%mutate(balance_f=balance_f/as.numeric(richness))
names(balance_dd)[names(balance_dd)=="balance_f"]<-"balance"

weighted_balance_dd<-complete_aggr%>%dplyr::select(balance_r,richness,CV, nutrients)%>%
  cbind(type="realised")
...
# Fit the linear model with interaction
lm_full_int1<-lm(data=complete_aggr_2,log10(stability)~log10(balance_f))
lm_full_int2<-lm(data=complete_aggr_2,log10(stability)~log10(balance_f) + log10(balance_r))
...
chi_sq_value <- anova5$"Sum of Sq"[2]
chi_sq_value
...
calculate_compositions <- function(data) {
  ...
  result <- data.frame(
    ...
    avg_slopes = sapply(combinations, function(comb) {
      mean(data$slope[data$predict_spec %in% comb])
    }),
...
all_combi_df<-all_combi_df%>%mutate(balance=abs(avg_slopes),
...
R2_env_bal<-data.frame(R2=c(sum_lm_balance$r.squared,sum_lm_env$r.squared,sum_lm_balance_env$r.squared,
                            ...
                            0),
                       model=factor(c("log10(imbalance)","nut * temp","log10(imbalance) + nut * temp","log10(imbalance)","nut * temp","log10(imbalance) + nut * temp")),
                       type=c("total","total","total","unique","unique","unique"))
 
 
plot_R2_bal_env<-ggplot(data=R2_env_bal%>%dplyr::filter(type=="total"),aes(y=R2,x=factor(model,levels=c("log10(imbalance)","nut * temp","log10(imbalance) + nut * temp")),fill=type))+
  geom_col(width=0.7)+
...
```{r}
anova_F <- car::Anova(model_rev3, type = "II")
# Convert to tidy format
...
## Including the effect of environmental conditions on stability
...
theme_classic()+
   theme(axis.title.x = element_text(size = 25),
    ...
    strip.text = element_text(size = 25)) +
   ylab("Temporal stability\nlog(mean/s.d.)")+
  xlab("log10(population stability)")+
  scale_color_viridis_d(option = "inferno", begin = 0.2, end = 0.8)
...
#ggsave(here("figures_ms", "fig4.png"), plot = fig4, width = 18, height = 12, dpi = 600)
...
knitr::include_graphics((here("figures_ms", "SEM_3.png")))

```
**Figure 16**: Structural equation model (SEM) showing the relationships between stability, asynchrony, population stability, imbalance, and nutrient levels. The model includes standardized regression coefficients and fit indices.
...
```{r}
# Extract the path (parameter estimates) from the lavaan object
path_table <- parameterEstimates(fit1C, standardized = TRUE) %>%
  dplyr::filter(op == "~") %>%  # Keep only regressions (paths)
  dplyr::mutate(
    Response = lhs,
    Predictor = rhs,
  ...
  dplyr::select(Response, Predictor, est, se, z, p_value, std.all)
...
path_table %>%
  ...
  tab_header(title = "Path Coefficients (SEM, lavaan)")
```
