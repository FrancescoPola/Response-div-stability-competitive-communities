...
```{r time_series_spp, fig.align="center", fig.height=20, fig.width=30}
...
ggplot(dens, aes(x = day, y = log(density), color = as.factor(nutrients))) +
  ...
  theme(
    ...
    # Rotate and increase axis text size
    ...
    # Move legend to top and make it horizontal
    legend.position = "top",
    legend.direction = "horizontal"
  ) +
  labs(color = "Nutrient level")
...
## GAMs to fit response surface 

We focus on the intrinsic rate of growth (r). This is because the intrinsic rate of growth has been shown to be a better predictor of community temporal stability than carrying capacity (K) (Ross et al 2023) [https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.14087]. 
We are going to fit response surfaces using the calculated r for all species using GAMs.  The rationale is that r is likely of more relevance if the environmental change of interest occurs rapidly, since r provides information on a population's ability to rapidly bounce back after disturbance. In the upcoming experiment, we will have temperature fluctuating relatively fast, and so we decide now to focus on r. 
...
Now we calculate RD with known trajectory of environmental change, i.e. the one we are going to apply in the experiment. 
...
```{r}
dist_slopes <- ggplot(data = df_slopes, aes(x = slope, y = rep(1, 45), color = species)) +
  geom_point(size = 5, alpha = 0.7) +  # translucent points
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  theme_bw() +
  facet_grid(nutrients ~ temperature, scales = "free_x") +  # allow x-axis to vary by column
...
## Calculate divergence for all possible compositions
...
plot_all_div <- ggplot(data = all_div, aes(x = richness, y = divergence, label = composition)) +
  ...
  theme_bw(base_size = 20) +  # Increase the base text size for theme elements
  theme(
